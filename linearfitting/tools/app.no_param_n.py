...
def caclR(startline,spliter,xysum,startcol,file_path,quiet):
...
def main(argv):
	...
	rvalue=1
	try:
		opts, args = getopt.getopt(argv,"hi:s:r:c:t:o:x:",["input_file=","xysum=","rr=","startcol=",'title=','operation=','rvalue='])
	...
	for opt, arg in opts:
		if opt == '-h':
			...
			print './app.py -i ../data/0505/2.csv -s 40(need x,y nums) -o 1(1:>rr;0:<rr) -r 0.5( rr) -x 1(1:r>0;0:r<0) -c 1(start cols)  -t(start rows) 1'
			sys.exit()
	...
	if cmp(input_file_path,'')!=0:		
		...
		if operation ==1:
			while True:
				i=i+1
				result=caclR(startrow,',',xysum,startcol,input_file_path,1)
